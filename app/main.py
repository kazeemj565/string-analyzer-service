...
@app.get("/strings", response_model=List[schemas.StringResponse])
def list_strings(
    is_palindrome: Optional[bool] = Query(None),
    min_length: Optional[int] = Query(None, ge=0),
    max_length: Optional[int] = Query(None, ge=0),
...
@app.get("/strings/filter-by-natural-language", response_model=List[schemas.StringResponse])
def filter_by_natural_language(q: str = Query(..., min_length=1), db: Session = Depends(get_db)):
    try:
        parsed = utils.parse_natural_language_query(q)
    except ValueError as e:
        raise HTTPException(status_code=400, detail=str(e))
